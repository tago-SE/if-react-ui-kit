...
function BarChart({ data, onBarClicked }) {
  const ref = useD3((svg) => {
...
const BarChartD3 = () => {
  ...
  const handleClick = () => { 
    const y = year + 1;
    setData([...data, { year: y, sales: 6081000, efficiency: 100}]);
    setYear(y);
  }

  const onBarClicked = (d) => {
    console.log("this bar was clicked!", d, d.target.id);
  }

  return (
    <div className="chart-container">
      <button onClick={handleClick}>test</button>
      <BarChart data={data} onBarClicked={onBarClicked} />
    </div>)
}
