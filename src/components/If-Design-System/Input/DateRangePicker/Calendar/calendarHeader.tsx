...
export function ArrowButton ({ variant, children, ...props } : IArrowButtonProps) {
    return (
        <button className={`if ${variant}`} type="button" {...props}>
          {children}
        </button>
    );
}
...
export const CalendarHeader = ({
    ...
    return (
      <div className="if header" style={{ marginBottom: "1rem" }}>
        <ArrowButton hidden={hidePrev} variant="previous" onClick={() => onPrev(prevDate)} aria-hidden={hidePrev} aria-label="Previous month">
            <span className="if axe sr-only">Previous Month, {getMonthName(prev.month)} {prev.year}</span>
        </ArrowButton>
        ...
        <ArrowButton hidden={hideNext} variant="next" onClick={() => onNext(nextDate)} aria-hidden={hideNext} aria-label="Next month">
            <span className="if axe sr-only">Next Month, {getMonthName(next.month)} {next.year}</span>
        </ArrowButton>
      </div>
    );
}